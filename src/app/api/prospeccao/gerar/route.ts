...
export async function POST(req: NextRequest) {
  ...
  const { tipo, regiao, nicho, quantidade } = await req.json()
  ...
  const objetivo = tipo === 'parcerias'
    ? `prospectar escritórios de contabilidade e contadores para fechar PARCERIAS de Escritório Virtual (o contador indica/oferece o endereço fiscal do Nex aos seus clientes). O e-mail deve focar no benefício mútuo e na comissão/recorrência para o parceiro.`
    : `prospecção comercial B2B do Nex Coworking — captar empresas que possam contratar salas privativas, estações de trabalho, salas de reunião, escritório virtual ou endereço fiscal.`
...
Formato do JSON:
{
  ...
  "emailTemplate": {
    ...
    "corpo": "e-mail de prospecção pronto, em PT-BR, com as variáveis {{nome}} e {{empresa}}, tom da marca Nex (próximo, direto, profissional), com CTA claro e assinatura 'Equipe Comercial Nex | comercial@nexcoworking.com.br'"
  }
}`
